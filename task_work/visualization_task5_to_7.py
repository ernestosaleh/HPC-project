...
ax.grid(True)
plt.savefig(f'images/task5_execution_time.png', bbox_inches='tight')
plt.close()
def amdahl(P, F):
    return 1 / (F/P + (1 - F))
...
ax.legend()
ax.grid(True)
plt.savefig(f'images/task5_amdahl.png', bbox_inches='tight')
plt.close()
...
ax.set(title=f'Execution time vs. number of processors', xlabel='number of processors', ylabel='Time (s)')
ax.grid(True)
plt.savefig(f'images/task6_execution_time.png', bbox_inches='tight')
plt.close()
def amdahl(P, F):
    return 1 / (F/P + (1 - F))
...
ax.legend()
ax.grid(True)
plt.savefig(f'images/task6_amdahl.png', bbox_inches='tight')
plt.close()
plt.show()
...
plt.savefig(f'images/task7_execution_time.png', bbox_inches='tight')
plt.close()